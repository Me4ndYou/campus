...
elseif ($mod == 'alternatif_tambah') {
    ...
    elseif ($db->get_results("SELECT * FROM tb_alternatif WHERE kode_alternatif='$kode_alternatif'"))
        ...
        $db->query("INSERT INTO tb_rel_alternatif(kode_alternatif, kode_kriteria, nilai) SELECT '$kode_alternatif', kode_kriteria, -1 FROM tb_kriteria");
...
elseif ($mod == 'kriteria_tambah') {
    ...
    if ($kode_kriteria == '' || $nama_kriteria == '' || $atribut == '' || $bobot == '')
        ...
        $db->query("UPDATE tb_kriteria SET nama_kriteria='$nama_kriteria', atribut='$atribut', bobot='$bobot' 
